...
import { FileText, CheckSquare, Square, ChevronDown, ChevronRight, Download, SlidersHorizontal } from 'lucide-react';
...
export const ExportManager: React.FC = () => {
  ...
  const handleExportTypst = () => {
    ...
    const typstContent = generateTypstDocument(stateSnapshot, {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800 tracking-tight">
              {t.export?.title || "Export dokumentu (Typst)"}
            </h2>
            <p className="text-xs text-slate-500 font-medium">
              {t.export?.desc || "Nakonfigurujte si rozsah exportovaného protokolu. Zapínejte a vypínejte jednotlivé sekce i vybraná podlaží a místnosti."}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-4 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition-colors flex items-center gap-2 ml-2"
          >
            <Download className="w-4 h-4" />
            {t.export?.exportBtn || "Exportovat do TYPST"}
          </button>
        </div>
      </div>
